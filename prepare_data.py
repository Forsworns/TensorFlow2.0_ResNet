...
import config
import pathlib
from config import image_height, image_width, channels, NUM_CLASSES, MAX_IMAGE_NUMBER
...
def get_images_and_labels(data_root_dir):
    ...
    # get labels' names
    label_names = sorted(
        item.name for item in data_root.glob('*/'))[:NUM_CLASSES]
    # dict: {label : index}
    label_to_index = dict((label, index)
                          for index, label in enumerate(label_names))
    all_image_path = list(filter(lambda p: pathlib.Path(
        p).parent.name in label_to_index, all_image_path))

    cnter = {}
    all_image_path_filtered = []
    for p in all_image_path:
        key = pathlib.Path(p).parent.name
        if key not in cnter:
            cnter.update({key: 0})
        if cnter[key] > MAX_IMAGE_NUMBER:
            continue
        cnter[key] += 1
        all_image_path_filtered.append(p)

    # get all images' labels
    all_image_label = [label_to_index[pathlib.Path(
        single_image_path).parent.name] for single_image_path in all_image_path_filtered]
    return all_image_path_filtered, all_image_label
...
def get_dataset(dataset_root_dir):
    ...
    image_dataset = tf.data.Dataset.from_tensor_slices(
        all_image_path).map(load_and_preprocess_image)
...
def generate_datasets():
    ...
    train_dataset = train_dataset.shuffle(
        buffer_size=train_count).batch(batch_size=config.BATCH_SIZE)
